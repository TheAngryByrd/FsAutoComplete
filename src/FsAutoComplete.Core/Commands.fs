...
module Commands =
  ...
  let pipelineHints (tryGetFileSource: _ -> Result<NamedText, _>) (tyRes: ParseAndCheckResults) =
    ...
    |> Result.either id (fun _ -> CoreResponse.InfoRes "Couldn't find file content")
  ...
  let symbolUseWorkspace
    ...
    asyncResult {
      ...
      | scope ->
        let projectsToCheck: Async<FSharpProjectOptions list> = async {
          match scope with
  ...
  member x.SymbolUseWorkspace
    ...
    asyncResult {
      ...
      let tryGetProjectOptionsForFsproj (fsprojPath: string<LocalPath>) =
        state.ProjectController.GetProjectOptionsForFsproj(UMX.untag fsprojPath) |> Async.singleton
