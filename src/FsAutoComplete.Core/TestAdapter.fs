...
let getExpectoTests (ast: ParsedInput) : TestAdapterEntry<range> list =
  ...
  let (|Case|List|NotExpecto|) =
    ...
    | SynExpr.LongIdent (_, SynLongIdent (lst, _, _), _, _) ->
      let i = lst |> List.last
  ...
  let visitModulesAndNamespaces prefix modulesOrNss =
    Seq.iter (fun (SynModuleOrNamespace (_, _, _, decls, _, _, _, _, _)) -> visitDeclarations prefix decls) modulesOrNss
...
let getNUnitTest (ast: ParsedInput) : TestAdapterEntry<range> list =
  ...
  let getName =
    function
    | SynPat.Named (ident = SynIdent(name, _)) -> name.idText
    | SynPat.LongIdent(longDotId = SynLongIdent (ident, _, _)) -> ident |> List.last |> (fun n -> n.idText)
    | _ -> ""
  ...
  let visitModulesAndNamespaces parent modulesOrNss =
    Seq.iter
      (fun (SynModuleOrNamespace (ids, _, _, decls, _, _, _, r, _)) ->
        let name = String.concat "." [ for i in ids -> i.idText ]
        ident <- ident + 1
